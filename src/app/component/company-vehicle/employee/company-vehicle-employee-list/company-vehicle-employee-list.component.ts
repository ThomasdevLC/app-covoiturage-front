...
export class CompanyVehicleEmployeeListComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    if (status) {
        console.log('status from URL:', status +' arr: '+ dateArrivee+' dep: '+dateDepart);
        this.companyVehicleEmployeeService.getCompanyVehicleByStatus(status).subscribe({
            next: (data) => {
                console.log('stat Données reçues:', data);
                this.vehicle = data;  // Assurez-vous que 'data' est bien un tableau de Vehicle
            },
            error: (err) => {
                console.error('Erreur lors de la récupération des données du véhicule', err);
            },
        });
        ...
        this.companyVehicleEmployeeService.getCompanyVehicleByStatusTime(status, arriveeDate, departDate).subscribe({
            next: (data) => {
                console.log('statTime Données reçues avec dates:', data);
                this.vehicle = data;  // Assurez-vous que 'data' est bien un tableau de Vehicle
            },
            error: (err) => {
                console.error('Erreur lors de la récupération des données du véhicule avec dates', err);
            },
        });
    } else {
