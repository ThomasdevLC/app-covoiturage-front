...
//private table2 = 'http://localhost:8080/vehicle_bookings';
  ...
  //ex: status/AVAILABLE
  getCompanyVehicleByStatus(status: string): Observable<Vehicle[]> {
  ...
  getCompanyVehicleByStatusTime(status: string, dateArrivee: Date, dateDepart: Date): Observable<Vehicle[]> {
    const arriveeStr = dateArrivee.toISOString().slice(0, 19);
    const departStr = dateDepart.toISOString().slice(0, 19);
    console.log('??status:' + status + ' url: ' + this.apiURL + ' date arr: ' + arriveeStr+ ' date dep: ' + departStr);
    //const isPossible :boolean;//pour encadrement des dates de depart et d'arrivee
    //if()
    return this.http.get<Vehicle[]>(`${this.apiURL}/status/${status}/arrivee/${arriveeStr}/depart/${departStr}`);
  }
  /*
